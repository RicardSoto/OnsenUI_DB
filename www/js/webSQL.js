    ...
    db.transaction(function (tx) {
        tx.executeSql("SELECT id,data FROM notes", [], function (sqlTransaction, sqlResultSet) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                x.innerHTML = '<ons-list-item><div class="listItem"><div>' + 
                cur_item.data + '</div><div style="width: 30%;"><ons-button id="' + 
                cur_item.id + '" onclick="showTemplateDialog(id,' + text + ')"><ons-icon icon="edit"></ons-icon></ons-button><ons-button style="background-color:orangered;margin-left: 15px;" id="' + 
                cur_item.id + '" onclick="deleteN(id)"><ons-icon icon="trash"></ons-icon></ons-button></div></div></ons-list-item>';
                wea.appendChild(x);
            }
            console.log('Done!!!');
...
function PopUp(id, value) {
    ...
    if (dialog) {
        ...
        e.innerHTML = '<ons-input id="newTextC" modifier="underbar" placeholder="Su Nota..." float></ons-input>'
    } else {
